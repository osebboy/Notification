...
class Dispatcher
{
	...
	public function notify($event, $args = null)
	{
		...
		{
			$list->push($this->invoke($observer, $args));
	...
	public function chain($event, $value)
	{
		foreach ($this->getObservers($event) as $observer)
		{
			$value = $this->invoke($observer, array($value));
		}
		return $value;
	}
	 ...
	 * @return mixed
	 */
	protected function invoke(array $observer, array $args = array())
	{
		$obj = $observer[0];
		if (is_string($obj))
		{
			$obj = empty($observer[2]) ? new $obj() : new $obj($observer[2]);
		}
		$method = $observer[1];
		switch (count($args))
		{
